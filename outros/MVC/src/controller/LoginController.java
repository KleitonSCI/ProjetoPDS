...
public class LoginController {
	...
	class LoginListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (command.equals("BotaoLoginAction")) {
				...
				String fone = view.getFone();

				// Atualiza e busca informações no modelo de dados
				salvar(login, senha,fone);
	...
	public void salvar(String login, String senha, String fone) {
		user.setLogin(login);
		user.setSenha(senha);
		user.setFone(fone);
	}
	...
	public void atualizarView() {
		view.mostrar(user.getLogin(), user.getSenha(), user.getFone());
	}
